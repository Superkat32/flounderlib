...
public class RepoText extends FlounderText {
    public static final Identifier ID = Identifier.fromNamespaceAndPath(FlounderLib.MOD_ID, "repo_text");
    ...
    public RepoText(Component text) {
        super(text);
        int textLength = text.tryCollapseToString().length();
        this.maxTicks = Math.min(8 + (textLength), 30);

        this.yellowTicks = 4;
        this.bounceInTicks = 8;
        this.bounceAmountY = Mth.randomBetween(this.random, 3f, 7f);
    }
